...
<main id="main-container">
            ...
            <div class="content content-top text-center overflow-hidden">
                <div class="pt-40 pb-20">
                    <h3 class="font-size-h2 font-w300 mt-20" data-toggle="appear" data-class="animated flipInY"
                        id="titulos" style="color: white;"><?php echo base64_decode($_GET['publicacion']); ?></h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <p style=" color: black; font-family: sans-serif;" class="text-center">
            A continuación se muestran los candidatos aplicado a la oferta de trabajo. Desde la más reciente a la más antigua.
    </div>
    </p>
    ...
    <div class="block">
       ...
       WHERE `IDpostulaciones` = ?";
         ...
         <tbody class="text-center">';
                    while ($item = $stmt->fetch()) {
                        ...
                        WHERE `IDUsuario` = ? AND  `FechaFinal` = (
                        SELECT MAX(`FechaFinal`)
                        FROM usuario_experiencia
                        WHERE `IDUsuario` = ?) 
                        LIMIT 0,1";
      ...
      <td>
      <a href="cv?id=' . base64_encode($item['IDUsuario']) . '"  class="bta-original-title="Ver Perfil del candidato"  target="_blank"> <i class="si si-usn btn-secondary text-center boton" data-toggle="tooltip" title="Ver Perfil del candidato" dater"></i></a>
      <a href="aceptar-candidato.php?id=' . base64_encode($item['IDUsuario']) . '' . base64_encode($_GET['id']) . '" class="btn btn-success btn-sm">Aceptar</a>
      <a href="denegar-candidato.php?id=' . base64_encode($item['IDUsuario']) . '&post=' . base64_encode($_GET['id']) . '" class="btn btn-danger btn-sm">Denegar</a>
      </td>
      </tr>';
                    }
